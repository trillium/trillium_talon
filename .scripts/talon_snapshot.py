...
# Orgs/users considered "owned" — repos whose origin matches these are forks
OWNED_ORGS = {"trillium"}

# Repos known to be safe to remove (no longer used)
REMOVABLE_REPOS = {"MouseControlChicken"}
...
def snapshot_git_repo(dir_path: str) -> dict:
    ...
    # Fork vs clone detection
    origin_url = remotes.get("origin", "")
    is_fork = any(org in origin_url for org in OWNED_ORGS)
    git["is_fork"] = is_fork

    # For plain clones (not forks), capture git diff vs HEAD to record local changes
    if not is_fork:
        # diff of tracked changes vs HEAD
        rc, diff_out = _git(["diff", "HEAD"], dir_path)
        git["local_diff"] = diff_out if rc == 0 and diff_out else None
...
def _summarize_dir(d: dict) -> dict:
    ...
    if d["type"] == "git":
        ...
        summary["is_fork"] = g.get("is_fork", False)
    elif d["type"] == "plain":
...
def _diff_git(ga: dict, gb: dict) -> dict:
    ...
    # Fork/clone classification and local diffs (plain clones only)
    d["is_fork_a"] = ga.get("is_fork", True)
    d["is_fork_b"] = gb.get("is_fork", True)
    if not d["is_fork_a"] or not d["is_fork_b"]:
        d["local_diff_a"] = ga.get("local_diff")
        d["local_diff_b"] = gb.get("local_diff")
...
def generate_reconciliation(snap_a: dict, snap_b: dict) -> str:
    ...
    if diff["only_in_a"] or diff["only_in_b"]:
        ...
        for name in diff["only_in_a"]:
            ...
            if name in REMOVABLE_REPOS:
                w(f"- **Status: CLEAR TO REMOVE** — no longer in use")
                w(f"- **Action on {host_a}:** `rm -rf ~/.talon/user/{name}`")
            elif detail["type"] == "git":
                origin = detail.get("remotes", {}).get("origin", "unknown")
                branch = detail.get("branch", "unknown")
                is_fork = detail.get("is_fork", False)
                repo_type = "fork (owned)" if is_fork else "plain clone"
                w(f"- Type: git repo ({repo_type}), branch `{branch}`")
                w(f"- Origin: `{origin}`")
        ...
        for name in diff["only_in_b"]:
            ...
            if name in REMOVABLE_REPOS:
                w(f"- **Status: CLEAR TO REMOVE** — no longer in use")
                w(f"- **Action on {host_b}:** `rm -rf ~/.talon/user/{name}`")
            elif detail["type"] == "git":
                origin = detail.get("remotes", {}).get("origin", "unknown")
                branch = detail.get("branch", "unknown")
                is_fork = detail.get("is_fork", False)
    ...
    # Local diffs for plain clones
    clone_diffs = []
    for name, entry in diff["directories"].items():
        if "git_diff" not in entry:
            continue
        gd = entry["git_diff"]
        diff_a = gd.get("local_diff_a")
        diff_b = gd.get("local_diff_b")
        if diff_a or diff_b:
            clone_diffs.append((name, diff_a, diff_b))
    ...
    if clone_diffs:
        ...
        for name, diff_a, diff_b in clone_diffs:
            ...
            if diff_a and diff_b:
                if diff_a == diff_b:
                    w(f"- Diffs are **identical** on both machines — no action needed")
                else:
                    w(f"- Diffs **differ** between machines — manual review required")
                    w(f"<details><summary>{host_a} diff</summary>")
                    w(f"")
                    w(f"```diff")
                    w(diff_a)
                    w(f"```")
                    w(f"</details>")
                    w(f"")
                    w(f"<details><summary>{host_b} diff</summary>")
                    w(f"")
                    w(f"```diff")
                    w(diff_b)
                    w(f"```")
                ...
                w(f"- Only **{host_a}** has local changes:")
                w(f"<details><summary>diff</summary>")
                w(f"")
                w(f"```diff")
                w(diff_a)
                w(f"```")
                ...
                w(f"- Only **{host_b}** has local changes:")
                w(f"<details><summary>diff</summary>")
                w(f"")
                w(f"```diff")
                w(diff_b)
