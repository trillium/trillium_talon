...
# PARROT_HOME = TALON_HOME / 'user' / 'spiteless_knausj_talon' / 'plugin' / 'parrot'
...
class Delegate(ParrotDelegate):
    ...
    def apply_patterns(self) -> None:
        ...
        for pattern in patterns_to_validate:
            ...
            missing_labels = pattern.labels - classes
            if len(missing_labels) > 0:
                invalid_patterns.add(pattern.name)
                logging.warning(f"[parrot] Pattern {repr(pattern.name)} has invalid labels: {missing_labels}")
    ...
    def pattern_match(self, frame: ParrotFrame) -> set[str]:
        ...
        for pattern in self.patterns.values():
            if pattern.detect(frame):
                ...
                winner_label, winner_prob = next(iter(frame.classes.items()))
                confidence_msg = f"\n{winner_prob * 100:.1f}%"
                # Store the confidence score globally so it can be accessed by other modules
                last_parrot_confidence = winner_prob * 100
                actions.user.notify(confidence_msg, 1)
                self.throttle_patterns(pattern.get_throttles(), frame.ts)

        return active

# Global variable to store the last detected parrot confidence score
